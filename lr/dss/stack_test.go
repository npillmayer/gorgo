...
func TestPush2(t *testing.T) {
	...
	s1.Push(1, A)
	if s1.tos.pathcnt != 1 {
		tracer().Errorf("pathcount at tos expected to be 1, is %d", s1.tos.pathcnt)
		t.Fail()
	}
	s2.Push(1, A)
	if s1.tos != s2.tos {
...
func TestPush3(t *testing.T) {
	...
	s1.Push(3, B)
	if s1.tos.pathcnt != 2 {
		tracer().Errorf("pathcount at join expected to be 2, is %d", s1.tos.pathcnt)
		t.Fail()
	}
}
...
func TestPush4(t *testing.T) {
	...
	s3.Push(3, C)
	if s1.tos.pathcnt != 3 {
		tracer().Errorf("pathcount at join expected to be 3, is %d", s1.tos.pathcnt)
		t.Fail()
	}
}
...
func TestPath1(t *testing.T) {
	...
	tracer().Debugf("path = %v", path)
	if path == nil || len(path) != 2 || path[1] == nil {
		tracer().Errorf("path not found or incorrect: ", path)
		t.Fail()
	}
}
...
func TestPath2(t *testing.T) {
	...
	tracer().Debugf("path = %v", path)
	if path == nil || len(path) != 2 || path[1] == nil {
		tracer().Errorf("path not found or incorrect: %v", path)
		t.Fail()
	}
	/*
		tmp, _ := ioutil.TempFile("", "stack_")
		tracer().Infof("writing DOT to %s", tmp.Name())
		DSS2Dot(r, path, tmp)
	*/
	path2 := s2.FindHandlePath(handle, 1)
	/*
		tracer().Debugf("path = %v", path2)
		tmp, _ = ioutil.TempFile("", "stack_")
		tracer().Infof("writing DOT to %s", tmp.Name())
		DSS2Dot(r, path2, tmp)
	*/
	if path[0] == path2[0] {
		tracer().Errorf("did not find 2nd handle")
		t.Fail()
	}
}
...
func TestPush5(t *testing.T) {
	...
	if len(path[0].succs) != 2 {
		tracer().Errorf("inverse fork at %v incorrect", path[0])
		t.Fail()
	}
	if !path[0].isInverseFork() {
		tracer().Errorf("%v not recognized as inverse fork", path[0])
		t.Fail()
	}
}
...
func TestSplitOff1(t *testing.T) {
	...
	s3 := s2.splitOff(path)
	if s2.tos == s3.tos {
		tracer().Errorf("stacks not split")
		t.Fail()
	}
	if len(r.stacks) != 2 {
		t.Fail()
...
func TestPush6(t *testing.T) {
	...
	if !s1.tos.isInverseJoin() || s1.tos.predecessorCount() != 3 {
		tracer().Errorf("3-join not correctly recognized")
		t.Fail()
	}
}
...
func TestReduce2(t *testing.T) {
	...
	if s1.tos != s2.tos {
		tracer().Errorf("TOS of stacks not merged")
		t.Fail()
	}
	if s1.tos.preds[0] == s1.tos.preds[1] {
		tracer().Errorf("no inverse join for symbol C in different states")
		t.Fail()
	}
	...
	if len(stacks) != 2 {
		tracer().Errorf("# of stack heads after reduce: %d", len(stacks))
		t.Fail()
	}
	//tmp, _ = ioutil.TempFile("", "stack_")
	//tracer().Infof("writing DOT to %s", tmp.Name())
